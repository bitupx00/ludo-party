...
/**
 * Board rotation (Ludo King style): each player sees their own color at the
 * BOTTOM-LEFT corner. Only the rendering rotates — logical positions (and
 * therefore online sync) are untouched.
 *
 * Unrotated corners: blue = top-left, red = top-right,
 *                    yellow = bottom-left, green = bottom-right.
 */
 ...
 *  after k quarter-turns. Unrotated home lanes (Ludo King): red arrives from
 *  the top arm, green from the right, yellow from the bottom, blue from the left. */
export function centerSideColors(k: number): [Color, Color, Color, Color] {
  const base: Color[] = ['red', 'green', 'yellow', 'blue'];
  const turns = ((k % 4) + 4) % 4;
