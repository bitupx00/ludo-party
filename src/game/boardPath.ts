...
/**
 * Ludo board path: 52 squares numbered 0–51, LUDO KING convention.
 *
 * The board is a cross with 4 arms on a 15×15 logical grid.
 * Bases: blue = top-left, red = top-right, yellow = bottom-left, green = bottom-right.
 *
 * Ludo King geometry (clockwise travel):
 *  - Each color EXITS onto the arm beside its base and walks around the ring.
 *  - Each color's HOME LANE is the center column/row of that same arm,
 *    entered from the arm's tip after 50 ring squares.
 *
 *  Entries:  blue (1,6) → east   | red (8,1) → south
 *            green (13,8) → west | yellow (6,13) → north
 *  Home-lane entrances (last ring square before the lane):
 *            blue (0,7)=idx 50 | red (7,0)=idx 11
 *            green (14,7)=idx 24 | yellow (7,14)=idx 37
 */
...
// 0–4: blue entry — east along left arm's top row: (1,6) → (5,6)
...
// 11: (7,0) — top tip (RED home-lane entrance)
BOARD_PATH.push({ col: 7, row: 0 });
// 12: (8,0)
BOARD_PATH.push({ col: 8, row: 0 });
// 13–17: red entry — south down the top arm's right column: (8,1) → (8,5)
...
// 24: (14,7) — right tip (GREEN home-lane entrance)
BOARD_PATH.push({ col: 14, row: 7 });
// 25: (14,8)
BOARD_PATH.push({ col: 14, row: 8 });
// 26–30: green entry — west along right arm's bottom row: (13,8) → (9,8)
...
// 37: (7,14) — bottom tip (YELLOW home-lane entrance)
BOARD_PATH.push({ col: 7, row: 14 });
// 38: (6,14)
BOARD_PATH.push({ col: 6, row: 14 });
// 39–43: yellow entry — north up the bottom arm's left column: (6,13) → (6,9)
...
// 50: (0,7) — left tip (BLUE home-lane entrance)
